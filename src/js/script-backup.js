...
function initializeNavigation() {
    // Initialize responsive navigation manager
    if (typeof ResponsiveNavigationManager !== 'undefined') {
        window.navManager = new ResponsiveNavigationManager();
    } else {
        // Fallback navigation for desktop links
...
// Fallback desktop navigation for when responsive nav isn't loaded
function initializeDesktopNavigation() {
    document.querySelectorAll('.nav-link, .desktop-nav-link').forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            document.querySelectorAll('.nav-link, .desktop-nav-link').forEach(l => l.classList.remove('active'));
...
// Unified navigation function for both mobile and desktop
function navigateToPage(pageId, viewMode = 'auto') {
    if (viewMode === 'auto') {
...
// Get current view mode based on screen size
function getCurrentViewMode() {
    return window.innerWidth >= 1024 ? 'desktop' : 'mobile';
}

// Update active navigation for the current view mode
function updateActiveNavigation(pageId, viewMode) {
    ...
    if (viewMode === 'mobile') {
        ...
        if (mobileBottomLink) mobileBottomLink.classList.add('active');
        if (mobileDrawerLink) mobileDrawerLink.classList.add('active');
    } else {
...
// Show content for the current view mode
